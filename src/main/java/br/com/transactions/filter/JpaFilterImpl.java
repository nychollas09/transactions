...
public abstract class JpaFilterImpl implements JPAFilter {
    ...
    @Override
    public JPAFilter addPredicate(
            final BiFunction<CriteriaBuilder, Root<?>, Predicate> predicate) {
    ...
    @Override
    public JPAFilter addGroupBy(
            Function<Root<?>, List<Expression<?>>> groupBy) {
    ...
    public Predicate getPredicate(CriteriaBuilder builder, Root<?> root) {
        ...
        return builder.and(getPredicates().stream()
                .map(f -> f.apply(builder, root)).collect(Collectors.toList())
